...
pub struct MusigSessionSecRand([u8; 32]);

impl MusigSessionSecRand {
    ...
    /// Creates a new [`MusigSessionSecRand`] with random bytes from the given rng
    #[cfg(feature = "rand")]
    pub fn from_rng<R: rand::Rng + ?Sized>(rng: &mut R) -> Self {
        let session_secrand = crate::random_32_bytes(rng);
        MusigSessionSecRand(session_secrand)
    }

    /// Creates a new [`MusigSessionSecRand`] with the given bytes.
    ///
    ...
    ///
    /// In rand-std environment, [`MusigSessionSecRand::new`] can be used to generate a random
    /// session id using thread rng.
    pub fn assume_unique_per_nonce_gen(inner: [u8; 32]) -> Self { MusigSessionSecRand(inner) }

    /// Obtains the inner bytes of the [`MusigSessionSecRand`].
    pub fn to_bytes(&self) -> [u8; 32] { self.0 }

    /// Obtains a reference to the inner bytes of the [`MusigSessionSecRand`].
...
/// * `session_secrand`: [`MusigSessionSecRand`] Uniform random identifier for this session. Each call to this
...
/// # use secp256k1_zkp::{PublicKey, Secp256k1, SecretKey, new_musig_nonce_pair, MusigSessionSecRand};
/// # let secp = Secp256k1::new();
/// // The session id must be sampled at random. Read documentation for more details.
/// let session_secrand = MusigSessionSecRand::new(&mut thread_rng());
...
///
/// let (_sec_nonce, _pub_nonce) = new_musig_nonce_pair(&secp, session_secrand, None, Some(sk), pk, None, None)
///     .expect("non zero session id");
/// # }
/// ```
pub fn new_musig_nonce_pair<C: Signing>(
    secp: &Secp256k1<C>,
    session_secrand: MusigSessionSecRand,
...
impl MusigKeyAggCache {
    ...
    /// # # [cfg(any(test, feature = "rand-std"))] {
    /// # use secp256k1_zkp::rand::{thread_rng, RngCore};
    /// # use secp256k1_zkp::{MusigKeyAggCache, Secp256k1, SecretKey, Keypair, PublicKey, MusigSessionSecRand, Message};
    /// # let secp = Secp256k1::new();
    /// # let sk1 = SecretKey::new(&mut thread_rng());
    /// # let pub_key1 = PublicKey::from_secret_key(&secp, &sk1);
    /// # let sk2 = SecretKey::new(&mut thread_rng());
    ...
    pub fn nonce_gen<C: Signing>(
        ...
        new_musig_nonce_pair(secp, session_secrand, Some(self), None, pub_key, Some(msg), extra_rand)
    }
...
impl MusigAggNonce {
    ...
    /// # # [cfg(any(test, feature = "rand-std"))] {
    /// # use secp256k1_zkp::rand::{thread_rng, RngCore};
    /// # use secp256k1_zkp::{MusigKeyAggCache, Secp256k1, SecretKey, Keypair, PublicKey, MusigSessionSecRand, Message, MusigAggNonce};
    /// # let secp = Secp256k1::new();
    /// # let sk1 = SecretKey::new(&mut thread_rng());
    /// # let pub_key1 = PublicKey::from_secret_key(&secp, &sk1);
    /// # let sk2 = SecretKey::new(&mut thread_rng());
    ...
    /// let msg = Message::from_digest_slice(b"Public Message we want to sign!!").unwrap();
    ///
    /// let session_secrand1 = MusigSessionSecRand::new(&mut thread_rng());
    ...
    /// let session_secrand2 = MusigSessionSecRand::new(&mut thread_rng());
...
impl MusigSession {
    ...
    /// # # [cfg(any(test, feature = "rand-std"))] {
    /// # use secp256k1_zkp::rand::{thread_rng, RngCore};
    /// # use secp256k1_zkp::{MusigKeyAggCache, Secp256k1, SecretKey, Keypair, PublicKey, MusigSessionSecRand, Message, MusigAggNonce, MusigSession};
    /// # let secp = Secp256k1::new();
    /// # let sk1 = SecretKey::new(&mut thread_rng());
    /// # let pub_key1 = PublicKey::from_secret_key(&secp, &sk1);
    /// # let sk2 = SecretKey::new(&mut thread_rng());
