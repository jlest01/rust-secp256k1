...
impl SessionSecretRand {
    ...
    pub fn from_rng<R: rand::Rng + ?Sized>(rng: &mut R) -> Self {
        ...
        SessionSecretRand{ data: session_secrand }
    }
    ...
    pub fn assume_unique_per_nonce_gen(inner: [u8; 32]) -> Self { SessionSecretRand{ data: inner } }

    /// Obtains the inner bytes of the [`SessionSecretRand`].
    pub fn to_bytes(&self) -> [u8; 32] { self.data }

    /// Obtains a reference to the inner bytes of the [`SessionSecretRand`].
    pub fn as_bytes(&self) -> &[u8; 32] { &self.data }
}
...
/// # use secp256k1_zkp::{PublicKey, Secp256k1, SecretKey, new_nonce_pair, SessionSecretRand};
...
pub fn new_nonce_pair<C: Signing>(
    ...
    unsafe {
        ...
        ) == 0
        {
            ...
            let pub_nonce = PublicNonce { data: pub_nonce.assume_init() };
            let sec_nonce = SecretNonce { data: sec_nonce.assume_init() };
            (sec_nonce, pub_nonce)
        }
    }
}
...
pub struct PartialSignature{ 
    data: ffi::MusigPartialSignature
}
...
impl PartialSignature {
    ...
    /// # Errors:
    ///
    /// - MalformedArg: If the signature [`PartialSignature`] is 32 bytes, but out of curve order
    pub fn from_slice(data: &[u8; ffi::MUSIG_PART_SIG_LEN]) -> Result<Self, ParseError> {
        ...
        unsafe {
            ...
            ) == 0
            {
                ...
                Ok(PartialSignature { data: partial_sig.assume_init() })
            }
        }
    }

    /// Get a const pointer to the inner PartialSignature
    pub fn as_ptr(&self) -> *const ffi::MusigPartialSignature { &self.data }

    /// Get a mut pointer to the inner PartialSignature
    pub fn as_mut_ptr(&mut self) -> *mut ffi::MusigPartialSignature { &mut self.data }
}
...
impl KeyAggCache {
    ...
    /// # # [cfg(any(test, feature = "rand-std"))] {
    /// # use secp256k1_zkp::rand::{thread_rng, RngCore};
    /// # use secp256k1_zkp::{KeyAggCache, Secp256k1, SecretKey, Keypair, PublicKey};
    /// # let secp = Secp256k1::new();
    /// # let sk1 = SecretKey::new(&mut thread_rng());
    /// # let pub_key1 = PublicKey::from_secret_key(&secp, &sk1);
    /// # let sk2 = SecretKey::new(&mut thread_rng());
    ...
    /// # # [cfg(any(test, feature = "rand-std"))] {
    /// # use secp256k1_zkp::rand::{thread_rng, RngCore};
    /// # use secp256k1_zkp::{KeyAggCache, Secp256k1, SecretKey, Keypair, PublicKey, SessionSecretRand, Message};
    /// # let secp = Secp256k1::new();
    /// # let sk1 = SecretKey::new(&mut thread_rng());
    /// # let pub_key1 = PublicKey::from_secret_key(&secp, &sk1);
    /// # let sk2 = SecretKey::new(&mut thread_rng());
...
impl SecretNonce {
    /// Get a const pointer to the inner KeyAggCache
    pub fn as_ptr(&self) -> *const ffi::MusigSecNonce { &self.data }

    /// Get a mut pointer to the inner KeyAggCache
    pub fn as_mut_ptr(&mut self) -> *mut ffi::MusigSecNonce { &mut self.data }
}
...
impl AggregatedNonce {
    ...
    /// # # [cfg(any(test, feature = "rand-std"))] {
    /// # use secp256k1_zkp::rand::{thread_rng, RngCore};
    /// # use secp256k1_zkp::{KeyAggCache, Secp256k1, SecretKey, Keypair, PublicKey, SessionSecretRand, Message, AggregatedNonce};
    /// # let secp = Secp256k1::new();
    /// # let sk1 = SecretKey::new(&mut thread_rng());
    /// # let pub_key1 = PublicKey::from_secret_key(&secp, &sk1);
    /// # let sk2 = SecretKey::new(&mut thread_rng());
    ...
    /// # }
    /// ```
    pub fn new<C: Signing>(secp: &Secp256k1<C>, nonce_ptrs: &[&PublicNonce]) -> Self {
        let mut aggnonce = MaybeUninit::<ffi::MusigAggNonce>::uninit();

        unsafe {
            let pubnonces: &[*const ffi::MusigPubNonce] =
                transmute::<&[&PublicNonce], &[*const ffi::MusigPubNonce]>(nonce_ptrs);
            ...
            ) == 0
            {
                ...
                AggregatedNonce { data: aggnonce.assume_init() }
            }
        }
    }
    ...
    pub fn from_slice(data: &[u8; ffi::MUSIG_AGGNONCE_SERIALIZED_LEN]) -> Result<Self, ParseError> {
        ...
        unsafe {
            ...
            ) == 0
            {
                ...
                Ok(AggregatedNonce { data: aggnonce.assume_init() })
            }
        }
    }

    /// Get a const pointer to the inner AggregatedNonce
    pub fn as_ptr(&self) -> *const ffi::MusigAggNonce { &self.data }

    /// Get a mut pointer to the inner AggregatedNonce
    pub fn as_mut_ptr(&mut self) -> *mut ffi::MusigAggNonce { &mut self.data }
}

/// A musig Singing session.
#[derive(Debug, Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct Session {
    data: ffi::MusigSession
}
...
impl Session {
    ...
    /// # # [cfg(any(test, feature = "rand-std"))] {
    /// # use secp256k1_zkp::rand::{thread_rng, RngCore};
    /// # use secp256k1_zkp::{KeyAggCache, Secp256k1, SecretKey, Keypair, PublicKey, SessionSecretRand, Message, AggregatedNonce, Session};
    /// # let secp = Secp256k1::new();
    /// # let sk1 = SecretKey::new(&mut thread_rng());
    /// # let pub_key1 = PublicKey::from_secret_key(&secp, &sk1);
    /// # let sk2 = SecretKey::new(&mut thread_rng());
    ...
    pub fn new<C: Signing>(
        ...
        unsafe {
            ...
            ) == 0
            {
                ...
                Session { data: session.assume_init() }
            }
        }
    }
    ...
    pub fn partial_sign<C: Signing>(
        ...
        keypair: &Keypair,
        key_agg_cache: &KeyAggCache,
    ) -> Result<PartialSignature, MusigSignError> {
        unsafe {
            let mut partial_sig = MaybeUninit::<ffi::MusigPartialSignature>::uninit();
    ...
    /// # }
    /// ```
    pub fn partial_sig_agg(&self, partial_sigs: &[&PartialSignature]) -> schnorr::Signature {
        let mut sig = [0u8; 64];
        unsafe {
    ...
    pub fn as_ptr(&self) -> *const ffi::MusigSession { &self.data }

    /// Get a mut pointer to the inner Session
    pub fn as_mut_ptr(&mut self) -> *mut ffi::MusigSession { &mut self.data }
}
