...
int main(void) {
    ...
    rustsecp256k1_v0_10_0_xonly_pubkey out_pubkeys[N_TX_OUTPUTS];
    rustsecp256k1_v0_10_0_xonly_pubkey *out_pubkeys_ptrs[N_TX_OUTPUTS];
    unsigned char output36[36];
    rustsecp256k1_v0_10_0_xonly_pubkey taproot_outputs[N_TX_INPUTS];
    rustsecp256k1_v0_10_0_pubkey plain_outputs[N_TX_INPUTS];
    ...
    rustsecp256k1_v0_10_0_xonly_pubkey tx_inputs[N_TX_INPUTS];
    rustsecp256k1_v0_10_0_xonly_pubkey tx_outputs[N_TX_OUTPUTS];
    int ret;
    size_t i;
    ...
    /*** Sending ***/
    {
        rustsecp256k1_v0_10_0_keypair sender_seckeys[N_TX_INPUTS];
        const rustsecp256k1_v0_10_0_keypair *sender_seckey_ptrs[N_TX_INPUTS];
        rustsecp256k1_v0_10_0_silentpayments_recipient recipients[N_TX_OUTPUTS];
        const rustsecp256k1_v0_10_0_silentpayments_recipient *recipient_ptrs[N_TX_OUTPUTS];
        rustsecp256k1_v0_10_0_xonly_pubkey generated_outputs[N_TX_OUTPUTS];
        rustsecp256k1_v0_10_0_xonly_pubkey *generated_output_ptrs[N_TX_OUTPUTS];
        ...
        for (i = 0; i < N_TX_INPUTS; i++) {
            ...
            ret = rustsecp256k1_v0_10_0_ec_pubkey_create(ctx, &pubkey, plain_seckeys[i]);
            assert(ret);
            ret = rustsecp256k1_v0_10_0_ec_pubkey_serialize(ctx, serialized_pubkey, &len, &pubkey, SECP256K1_EC_COMPRESSED);
            assert(ret);
        ...
        ret = rustsecp256k1_v0_10_0_silentpayments_test_outputs(
            ctx,
            out_pubkeys_ptrs,
            recipient_ptrs, 
            N_TX_OUTPUTS,
        ...
        for (i = 0; i < N_TX_OUTPUTS; i++) {
            ...
            len = 33;
            ret = rustsecp256k1_v0_10_0_ec_pubkey_serialize(ctx, serialized_pubkey, &len, &recipients[i].scan_pubkey, SECP256K1_EC_COMPRESSED);

            ret = rustsecp256k1_v0_10_0_xonly_pubkey_serialize(ctx, serialized_xonly_pubkey, &out_pubkeys[i]);
            assert(ret);
        ...
        for (i = 0; i < N_TX_INPUTS; i++) {
            rustsecp256k1_v0_10_0_xonly_pubkey orignal_pubkey;
            ...
            ret = rustsecp256k1_v0_10_0_keypair_xonly_pub(ctx, &orignal_pubkey, NULL, &sender_seckeys[i]);
            assert(ret);
            ret = rustsecp256k1_v0_10_0_xonly_pubkey_serialize(ctx, serialized_original_xonly_pubkey, &orignal_pubkey);
            assert(ret);

            ret = rustsecp256k1_v0_10_0_xonly_pubkey_serialize(ctx, serialized_xonly_pubkey, &taproot_outputs[i]);
            assert(ret);
            ...
            ret = rustsecp256k1_v0_10_0_ec_pubkey_serialize(ctx, serialized_pubkey, &len, &plain_outputs[i], SECP256K1_EC_COMPRESSED);
            assert(ret);
    ...
    /*** Receiving ***/
    {
         ...
         */
        const rustsecp256k1_v0_10_0_xonly_pubkey *tx_input_ptrs[N_TX_INPUTS];
        const rustsecp256k1_v0_10_0_xonly_pubkey *tx_output_ptrs[N_TX_OUTPUTS];
...

        {
             ...
             *
             */
            rustsecp256k1_v0_10_0_silentpayments_found_output found_outputs[N_TX_OUTPUTS];
            rustsecp256k1_v0_10_0_silentpayments_found_output *found_output_ptrs[N_TX_OUTPUTS];
